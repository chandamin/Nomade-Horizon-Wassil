 ...
 * Must match the key used in ConsentBanner.jsx (`nh_analytics_consent`)
 */
export const hasAnalyticsConsent = () => {
  return localStorage.getItem("nh_analytics_consent") === "granted";
};

/**
 * Convert BigCommerce product options array to GA variant string.
 * e.g., [{name: "Color", value: "Noir"}, {name: "Size", value: "36"}] → "Noir, 36"
 */
export const formatVariant = (options = []) => {
  if (!Array.isArray(options)) return "Defaut";
  const values = options.filter(opt => opt?.value).map(opt => opt.value);
  return values.length > 0 ? values.join(", ") : "Defaut";
};

/**
 * Normalize address data from checkout form or BigCommerce order.
 */
...
/**
 * Determine if customer is new based on session state.
 */
...
/**
 * Return past order count for returning customer.
 */
...
/**
 * Map BigCommerce order products + cart line items to GA `items[]` array.
 * Uses server-verified prices from order, but enriches metadata from cart.
 */
export const buildGAItems = (orderProducts, cartLineItems = {}) => {
  // Strictly enforce array type to prevent `.map` errors
...
/**
 * Assemble full GA4 purchase payload with consent-aware PII handling.
 */
export const buildGAPayload = ({ 
  ...
  // Strictly enforce array type for products
  const productsArray = Array.isArray(order?.products) ? order.products : [];

  // Pass the safe array to buildGAItems
  ...
  //  Use the safe array for reduce calculations
  ...
  // 🔐 CONSENT FILTER: Remove PII if user has not consented
  if (!consentGranted) {
...
/**
 * Fire GA event to dataLayer with optional sendBeacon fallback for tab-close resilience.
 */
export const fireGAEvent = (payload) => {
  try {
    // Primary: sync dataLayer push (non-blocking)
    window.dataLayer = window.dataLayer || [];
    window.dataLayer.push(payload);
    console.debug("GA event fired:", payload.ecommerce?.transaction_id);
    
    // Fallback: sendBeacon for tab-close resilience
    if (navigator.sendBeacon) {
      ...
      navigator.sendBeacon(
        "/api/ga-ping", // optional backend logging endpoint
